...
function FirstAccessScreen({ account, onDone }){
  ...
  function submit(e){
    e.preventDefault();
    if(!p1 || p1.length<6 || p1!==p2){ setErr(true); return; }
    setErr(false);
    onDone(p1);
  }
  return (
    <div id="first-access-screen" className="show">
      <div className="login-form" style={{background:'var(--card)', padding:'38px', borderRadius:'20px', boxShadow:'var(--shadow)', maxWidth:'400px', width:'100%'}}>
        ...
        {err && <div className="login-error" style={{display:'block'}}>As senhas não coincidem ou são muito curtas (mín. 6 caracteres).</div>}
